...
class MaterialRequest(BuyingController):
	...
	def on_submit(self):
		...
		if self.material_request_type == 'New (Purchase)':
	...
	def update_completed_qty(self, mr_items=None, update_modified=True):
		...
		for d in self.get("items"):
			if d.name in mr_items:
				if self.material_request_type in ("Re-Work", "Old (Canab Unit)"):
...
def make_purchase_order(source_name, target_doc=None):
	...
	doclist = get_mapped_doc("Material Request", source_name, 	{
		"Material Request": {
			...
			"validation": {
				"docstatus": ["=", 1],
				"material_request_type": ["=", "New (Purchase)"]
			}
		},
	...
	if supplier_items:
		material_requests = frappe.db.sql_list("""select distinct mr.name
			...
			where mr.name = mr_item.parent
				...
				and mr.material_request_type = 'New (Purchase)'
...
def make_stock_entry(source_name, target_doc=None):
	def update_item(obj, target, source_parent):
		...
		if source_parent.material_request_type == "Old (Canab Unit)":
	...
	doclist = get_mapped_doc("Material Request", source_name, {
		"Material Request": {
			...
			"validation": {
				"docstatus": ["=", 1],
				"material_request_type": ["in", ["Old (Canab Unit)", "Re-Work"]]
			}
		},
		"Material Request Item": {
