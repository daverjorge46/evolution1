...
class Project(Document):
	...
	def validate_project_name(self):
		if self.get("__islocal") and frappe.db.exists("Project", self.project_name):
			frappe.throw(_("Work Order {0} already exists").format(frappe.safe_decode(self.project_name)))
	...
	def send_welcome_email(self):
		...
		messages = (
			_("You have been invited to collaborate on the Work Order: {0}".format(self.name)),
			url,
			_("Join")
		)
		...
		for user in self.users:
			if user.welcome_email_sent == 0:
				frappe.sendmail(user.user, subject=_("Work Order Collaboration Invitation"),
...
def send_project_status_email_to_users():
	...
	for d in frappe.get_all("Project Update",
		...
		args = {
			"users": doc.users,
			"title": _("Work Order Summary for {0}").format(yesterday)
		}

		frappe.sendmail(recipients=get_users_email(project_doc),
			template='daily_project_summary',
			args=args,
			subject=_("Daily Work Order Summary for {0}").format(d.name),
